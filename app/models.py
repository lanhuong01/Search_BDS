...
class BDS(models.Model):
    ...
    news_type = models.TextField('Loại tin',max_length=50,choices=news_type_CHOICES)
    price = models.FloatField('Giá')
    size = models.FloatField('Diện tích (mét vuông)')
    direction= models.TextField('Hướng nhà',max_length=50, choices=direction_CHOICES,blank=True, null=True)
    balcony_direction = models.TextField('Hướng ban công',max_length=50, 
                        choices=balcony_direction_CHOICES, blank=True, null=True)
    legal = models.TextField('Giấy tờ pháp lý',max_length=50, choices=legal_CHOICES)
    ...
    furniture = models.TextField('Nội thất',max_length=200)
    url = models.TextField('URL')
    location = models.TextField('Địa chỉ',max_length=200)
    description = models.TextField('Mô tả')
    range_price= models.TextField('Khoảng giá',choices=price_CHOICES, default=price_CHOICES[0][0])
    range_size= models.TextField('Khoảng diện tích',choices=size_CHOICES, default=size_CHOICES[0][0])
    city=models.TextField('Thành phố',max_length=200, default="Unknown")
